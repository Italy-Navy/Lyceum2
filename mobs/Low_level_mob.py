...
class LowLevelMob(sprite.Sprite):
    ...
    def low_level_mob_behavior(self, hero_x, hero_y, platforms):
        if self.rect.x < hero_x - (32 * 20) or self.rect.x > hero_x + (32 * 20):
            ...
            self._isdo = True
            if -50 <= int(hero_y) + 15 - self.rect.y <= 50 and 0 <= int(hero_x) - int(
                    self.rect.x) <= 20 and self.POSITION_RIGHT:
                self.xvel = 0
                self.image.fill(Color(COLOR))
            ...
            elif -50 <= int(hero_y) + 15 - self.rect.y <= 50 and 0 <= int(self.rect.x) - int(
                    hero_x) <= 20 and not self.POSITION_RIGHT:
                self.xvel = 0
                self.POSITION_RIGHT = False
                self.image.fill(Color(COLOR))
